    ...
    return (
        <div className={`bg-white p-3 rounded-lg text-sm flex container mx-auto ${className}`}>
            ...
            <div className="w-1/2 flex flex-wrap justify-end text-right">
                ...
                <select
                    className="rounded-lg px-1 py-1 bg-slate-300 text-white cursor-pointer outline-none" value={selectCurrency} 
                    onChange={(e) => onCurrencyChange && onCurrencyChange(e.target.value)} disabled={currencyDisable}
                >
                    
                    {currencyOptions?.map((currency) => (
                        <option className="bg-slate-200 text-black" key={currency} value={currency}> {currency} </option>
